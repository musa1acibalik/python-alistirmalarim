...
class ShoopingCart:
    ...
    def get_coupon(cls,code):
        return next(filter(lambda c:c.code==code,ShoopingCart.coupon_list))
    
    def apply_coupon(self,code):
        if code not in ShoopingCart.get_coupons():
            raise ValueError(f"geçersiz kode {code}")
        
        coupon=ShoopingCart.get_coupon(code)

        for index in range(0,len(self.liste)):
            self.liste[index].price=self.liste[index].price * coupon.discount

    
sc=ShoopingCart([item1,item2])
sc.add_item(item3)
sc.display_items()
